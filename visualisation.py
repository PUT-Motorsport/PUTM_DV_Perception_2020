...
def get_row(cone):
  row = {}
  row['color'] = colors[cone['color']]
  row['mean'] = np.mean(cone['intensity'])
  row['median'] = np.median(cone['intensity'])
  row['std'] = np.std(cone['intensity'])
  row['min'] = np.min(cone['intensity'])
  row['max'] = np.max(cone['intensity'])
  row['25%'] = np.quantile(cone['intensity'], 0.25)
  row['50%'] = np.quantile(cone['intensity'], 0.5)
  row['75%'] = np.quantile(cone['intensity'], 0.75)
  return row
...
i = 2
plt.scatter(df.loc[i]['x'], df.loc[i]['z'], c=df.loc[i]['intensity'], cmap='rainbow')
plt.ylim(-0.35, 0.3)
plt.title(colors[df.loc[i]['color']])
plt.show()

'''
columns = ['color', 'mean', 'median', 'std', 'min', 'max', '25%', '50%', '75%']

# intensity_df = pd.DataFrame(columns=columns)
...
# intensity_df.to_feather('intensity_df.feather')

intensity_df = read_feather('intensity_df.feather')

intensity_df.boxplot(column=columns[1:], by='color')
# plt.show()

le = LabelEncoder()
le.fit(intensity_df.iloc[:, 0])
target = le.transform(intensity_df.iloc[:, 0])

X_train, X_test, y_train, y_test = train_test_split(intensity_df.iloc[:, 1:6], target, test_size=0.2, random_state=42,
                                                    stratify=target)
